...
const SessionSettings: React.FC = () => {
  ...
  const onFinish = async (values: FormValues): Promise<void> => {
    try {
      ...
      void message.success("Đã kích hoạt đợt đăng ký đề tài mới!");
      form.resetFields();
      void fetchData();
    } catch {
      void message.error("Lỗi khi tạo cấu hình");
    }
  };
  ...
  const handleDelete = async (id: number): Promise<void> => {
    try {
      ...
      void message.success("Đã xóa đợt đăng ký");
      void fetchData();
    } catch {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (
        ...
        <Popconfirm
          title="Bạn có chắc muốn xóa đợt đăng ký này?"
          onConfirm={() => {
  ...
  return (
    <div style={{ padding: "24px", background: "#f5f7fa", minHeight: "100vh" }}>
      <Title level={3} style={{ marginBottom: 24, color: "#1e3c72" }}>
        ⚙️ Cấu hình đợt đăng ký Đề tài
      </Title>
      ...
      <Row gutter={24}>
          ...
          <Card
            title={
              <span style={{ color: "#1e3c72", fontWeight: "bold" }}>
                <PlusOutlined /> Thiết lập đợt đăng ký mới
              </span>
            }
            bordered={false}
            style={{
        ...
        <Col xs={24} lg={14}>
          <Card
            title={
              <span style={{ color: "#1e3c72", fontWeight: "bold" }}>
                <CalendarOutlined /> Các đợt đăng ký đang hoạt động
              </span>
            }
            bordered={false}
            style={{
          ...
          >
            ...
            >
              <Text
                type="warning"
                style={{ color: "#d46b08", fontSize: "13px" }}
              >
                💡 <strong>Lưu ý:</strong> Khi kết thúc đợt đăng ký, sinh viên
                sẽ không thể đăng ký hoặc thay đổi đề tài của họ nữa.
              </Text>
            </div>
          </Card>
        </Col>
