...
// --- Định cấu trúc các Interface dữ liệu chặt chẽ ---
...
const LecturerProfile: React.FC = () => {
  ...
  const fetchProfile = async (): Promise<void> => {
    setLoading(true);
    try {
      const res = (await getProfile()) as LecturerProfileData;
      setProfile(res);
      form.setFieldsValue({
        phone: res.phone || "",
        degree: res.degree || "",
      ...
      void message.error("Lỗi khi tải thông tin hồ sơ");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (values: FormValues): Promise<void> => {
    try {
      ...
      void message.error("Cập nhật hồ sơ thất bại");
    }
  };

  const saveTagsDirectly = async (newTags: string[]): Promise<void> => {
    try {
      await updateProfile({
      ...
      void message.success("Đã cập nhật định hướng nghiên cứu!");
    } catch {
      void message.error("Không thể cập nhật tag");
    }
  };

  // Add & Delete research direction tag handlers
  const handleCloseTag = (removedTag: string): void => {
    const newTags = tags.filter((tag) => tag !== removedTag);
    setTags(newTags);
    if (!isEditing) {
      // Auto-save tag modifications if not editing form
      void saveTagsDirectly(newTags);
    }
  };
                ...
                >
                  <Row gutter={[16, 16]}>
                    <Col xs={24} sm={12}>
                      <Form.Item
                        label={
                          <Text strong style={{ color: "#4f5d75" }}>
                      ...
                      >
                        <Input
                          placeholder="Ví dụ: Tiến sĩ, Phó Giáo sư..."
                          style={{ borderRadius: "8px" }}
                        />
                      </Form.Item>
                    </Col>
        ...
        <Col xs={24} lg={8}>
          ...
          >
            ...
            <div style={{ textAlign: "center", marginTop: "16px" }}>
              ...
              <div
                style={{ color: "#8c8c8c", fontSize: "12px", marginTop: "8px" }}
              >
                * Đếm theo tổng số lượng sinh viên đã phê duyệt đề tài và đang
                thực hiện trong học kỳ này.
              </div>
            </div>
          </div>
        </Col>
