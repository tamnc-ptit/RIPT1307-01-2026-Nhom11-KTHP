...
// --- Định nghĩa Interfaces chặt chẽ ---
interface ClassItem {
  id: number;
  class_name: string;
  course_name: string;
...
// Cấu trúc phản hồi chuẩn từ API getStudentsWithThesis
...
const ClassDiscussion: React.FC = () => {
  ...
  const fetchClasses = async (): Promise<void> => {
    ...
    try {
      ...
      console.error(err);
      void message.error("Không thể tải danh sách lớp");
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchStudents = async (classId: number): Promise<void> => {
    setLoading(true);
    try {
      const res = (await getStudentsWithThesis(classId)) as StudentsApiResponse;
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: string): React.ReactNode =>
        text || <Tag color="red">Chưa có đề tài</Tag>,
    },
    {
        ...
        <Button
          ...
          onClick={() => {
            if (record.latest_submission_id) {
              ...
              void message.warning("Sinh viên này chưa có bài nộp");
            }
          }}
        >
          Nhận xét
  ...
  return (
    <div style={{ padding: 24, background: "#f5f7fa", minHeight: "100vh" }}>
      <Space direction="vertical" size="large" style={{ width: "100%" }}>
        <Card
          title={
            <Title level={3} style={{ margin: 0 }}>
              💬 Diễn đàn bài nộp của sinh viên
            </Title>
          }
          extra={
            <Button
        ...
        >
          <Text>
            Chọn lớp, xem danh sách sinh viên cùng đề tài, sau đó chọn sinh viên
            để xem diễn đàn bài nộp của họ.
          </Text>
        </Card>
            ...
            >
              <div>
                <Title level={4} style={{ margin: 0 }}>
                  <CommentOutlined /> Danh sách sinh viên
                </Title>
                <Text type="secondary">
                  Đang quản lý:{" "}
                  {selectedClass ? selectedClass.class_name : "Chưa chọn lớp"}
                </Text>
              </div>
              <Select
                ...
                style={{ minWidth: 220 }}
                onChange={(value: number) => setSelectedClassId(value)}
                options={classes.map((item) => ({
                  label: `${item.class_name} ${item.course_name ? `- ${item.course_name}` : ""}`,
                  value: item.id,
                }))}
              />
            </div>
