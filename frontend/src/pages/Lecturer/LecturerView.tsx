...
// --- Định cấu trúc Hệ thống Interface Chặt chẽ ---
...
// Cấu trúc thực thể dữ liệu đề tài thô nhận về từ API Hệ thống
interface RawThesisItem {
  id: number;
  title: string;
  studentName?: string;
  class_id?: number;
...
// Cấu trúc phản hồi kỳ vọng từ API getLecturerTheses
interface ThesisApiResponse {
  items?: RawThesisItem[];
  [key: string]: unknown;
}

// Cấu trúc phản hồi kỳ vọng từ API getRiskFlags
...
const LecturerDashboard: React.FC = () => {
  ...
  const [allTheses, setAllTheses] = useState<RawThesisItem[]>([]);
  ...
  const fetchDashboardData = async (): Promise<void> => {
    ...
    try {
      ...
      ])) as [any, RawRiskResponse[], ThesisApiResponse];
      ...
      const itemsList = thesisRes.items || [];
      setAllTheses(itemsList);

      setTopics(
        itemsList.slice(0, 5).map((t) => ({
          id: t.id.toString(),
          title: t.title,
  ...
  const handleApproveQuick = async (thesisId: string): Promise<void> => {
    try {
      await approveThesis(parseInt(thesisId));
      void message.success("Đã duyệt đề tài thành công!");
      void fetchDashboardData();
    } catch {
      void message.error("Lỗi khi duyệt đề tài");
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "classCode",
      key: "classCode",
      render: (text: string): React.ReactNode => <Tag color="blue">{text}</Tag>,
    },
    {
      title: "Sinh viên",
    ...
    },
    {
      title: "Thao tác",
      key: "action",
      render: (unknownText: unknown, record: ThesisTopic): React.ReactNode => (
        <Space size="middle">
          <Button
            type="link"
            size="small"
          ...
          {record.status === "PENDING" && parseInt(record.id) > 0 && (
            <Button
              ...
              size="small"
              onClick={() => {
                void handleApproveQuick(record.id);
              }}
            >
              Duyệt nhanh
            </Button>
  ...
  const studentTheses = allTheses.filter(
    (t) => t.lecturer_status === "approved",
  );
        ...
        >
          <Col>
            <Title level={2} style={{ margin: 0, color: "#ffffff" }}>
              👋 Xin chào, Thầy {lecturerName}
            </Title>
            <Text style={{ color: "#e0e6ed", fontSize: "15px" }}>
              Dưới đây là thông tin tổng quan về các lớp hướng dẫn của bạn trong
              học kỳ này.
            </Text>
          </Col>
          <Col>
            ...
            >
              ...
              >
                ...
                >
                  {risks.length === 0 ? (
                    <Text type="secondary">
                      Tuyệt vời! Không có rủi ro trễ hạn nào.
                    </Text>
                  ) : (
                    <ul
                      style={{
                    ...
                    >
                      {risks.map((risk, idx) => (
                        <li key={idx} style={{ marginBottom: 8 }}>
                          <Text strong style={{ color: "#820014" }}>
                            {risk.studentName}
                          </Text>
                          <div style={{ fontSize: "12px", color: "#a61d24" }}>
                            {risk.riskType}
                          </div>
                        </li>
                      ))}
